...
const HomeScreen: React.FC<Props> = ({ navigation }) => {
  ...
  function _filterData(val) {
    if (val === 0) {
      ...
      let blogPosts = _.sortBy([...data], function (o) {
        return new moment(o.releaseDate);
      }).reverse();
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate("HomeDetail", { musicDetail: item })}
  ...
  function _renderNoDataView(image, text, srchTxt) {
    return (
      <View style={styles.noDataVw}>
        <Image style={styles.homeImage} source={image} />
        <Text style={styles.noDataTxt}>
          {srchTxt ? text.replace(/xyz/g, srchTxt) : text}
        </Text>
      </View>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      {_searchBar()}
      {srchTxt && musicData.length > 0 ? (
        <View style={styles.topTabVw}>
          <TouchableOpacity
            onPress={() => {
              setFilterOption(0), _filterData(0);
            }}
          ...
          >
            <Text
              style={[
                styles.topBtnTxt,
                { color: filterOption === 0 ? Colors.green : Colors.white },
              ]}
            >
              {String.ColName}
            </Text>
          ...
          <TouchableOpacity
            onPress={() => {
              setFilterOption(1), _filterData(1);
            }}
            style={styles.topTabBar}
          >
            <Text
              style={[
                styles.topBtnTxt,
                { color: filterOption === 1 ? Colors.green : Colors.white },
              ]}
            >
              {String.RelDate}
            </Text>
